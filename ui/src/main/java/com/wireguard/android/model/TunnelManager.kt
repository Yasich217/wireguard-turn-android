...
class TunnelManager(
    ...
    private suspend fun reconcileTurnForRunningTunnels(running: Set<String>) {
        ...
        for (tunnel in tunnelMap) {
            ...
            val started = withContext(Dispatchers.IO) {
                getTurnProxyManager().onTunnelEstablished(
                    tunnel.name,
                    turn,
                    publicKey,
                    keepaliveSec,
    ...
    suspend fun setTunnelConfig(
        ...
        if (originalState == Tunnel.State.UP) {
            setTunnelState(tunnel, Tunnel.State.UP, forceTurnStart = false, reconnecting = true)
        }
    ...
    suspend fun setTunnelState(tunnel: ObservableTunnel, state: Tunnel.State): Tunnel.State = withContext(Dispatchers.Main.immediate) {
        setTunnelState(tunnel, state, forceTurnStart = false, reconnecting = false)
    }
    ...
    suspend fun setTunnelState(tunnel: ObservableTunnel, state: Tunnel.State, forceTurnStart: Boolean): Tunnel.State = withContext(Dispatchers.Main.immediate) {
        setTunnelState(tunnel, state, forceTurnStart, reconnecting = false)
    }

    suspend fun setTunnelState(
        tunnel: ObservableTunnel,
        state: Tunnel.State,
        forceTurnStart: Boolean,
                ...
                if (shouldStartTurn) {
                    ...
                    if (useTurnForThisStart) {
                        getTurnProxyManager().setRuntimeStatusModeInfo(
                            tunnel.name,
                            context.getString(
                                if (reconnecting) R.string.turn_mode_retrying_proxy
                                else R.string.turn_mode_starting_proxy,
                            ),
                        )
                    ...
                    turnAutoState.remove(tunnel.name)
                    withContext(Dispatchers.IO) {
                        getTurnProxyManager().stopForTunnel(tunnel.name, clearRuntimeStatus = true)
                    }
                }
            }
                ...
                try {
                    ...
                    if (hasAnyHandshake(stats)) {
                        ...
                        if (turn != null && turn.enabled && turn.autoSwitchTurn) {
                            scheduleTurnAutoReevaluation(tunnel, immediate = false, reason = TurnReevalReason.POST_HANDSHAKE)
                        }
        ...
        underlyingNetworkRefreshJob = applicationScope.launch(Dispatchers.IO) {
            ...
            tunnelMap.toList().forEach { tunnel ->
                ...
                if (tunnel.state == Tunnel.State.UP && turn != null && turn.enabled && turn.autoSwitchTurn) {
                    scheduleTurnAutoReevaluation(tunnel, immediate = true, reason = TurnReevalReason.NETWORK_CHANGE)
                }
            }
        }
    }

    private enum class TurnReevalReason {
        POST_HANDSHAKE,
        NETWORK_CHANGE,
    }

    private fun scheduleTurnAutoReevaluation(tunnel: ObservableTunnel, immediate: Boolean, reason: TurnReevalReason) {
        cancelTurnAutoWatcher(tunnel.name)
        val job = applicationScope.launch(Dispatchers.IO) {
            val delayMs = when {
                immediate -> 0L
                reason == TurnReevalReason.POST_HANDSHAKE -> TURN_AUTO_POST_HANDSHAKE_DELAY_MS
                else -> TURN_AUTO_REEVALUATE_DELAY_MS
            }
            if (delayMs > 0L) {
                delay(delayMs)
            }
            try {
                maybeReevaluateTurnForTunnel(tunnel, immediate, reason)
            } catch (e: Throwable) {
    ...
    private suspend fun maybeReevaluateTurnForTunnel(tunnel: ObservableTunnel, immediate: Boolean, reason: TurnReevalReason) {
        ...
        if (!immediate && !networkChanged && now - previous.lastCheckElapsedMs < TURN_AUTO_REEVALUATE_INTERVAL_MS) {
            return
        }
        ...
        val reasonText = if (networkChanged || reason == TurnReevalReason.NETWORK_CHANGE) {
            "underlying network changed"
        } else {
            "post-handshake reevaluation"
        }
        Log.i(TAG, "TURN auto-switch reevaluation for ${tunnel.name}: shouldUseTurn=$shouldUseTurn current=$currentlyUsingTurn ($reasonText)")
    ...
    private suspend fun shouldUseTurnByAutoSwitch(turn: TurnSettings): Boolean = withContext(Dispatchers.IO) {
        val googleReachable = probeUrl(PROBE_GLOBAL_URL)
        val regRuReachable = probeUrl(PROBE_REG_RU_URL)
        Log.i(TAG, "TURN auto-switch probes: google=$googleReachable reg.ru=$regRuReachable")

        if (googleReachable && regRuReachable) {
            Log.i(TAG, "TURN auto-switch: google+reg.ru reachable via non-VPN network, TURN disabled")
            return@withContext false
        }

        Log.w(TAG, "TURN auto-switch: direct path checks are insufficient, keeping TURN enabled")
        true
    }
    ...
    companion object {
        ...
        private const val PROBE_REG_RU_URL = "https://www.reg.ru"
        ...
        private const val TURN_AUTO_POST_HANDSHAKE_DELAY_MS = 10 * 60 * 1000L
