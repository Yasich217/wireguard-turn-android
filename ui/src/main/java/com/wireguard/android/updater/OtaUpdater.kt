...
object OtaUpdater {
    ...
    private fun openOtaConnection(
        ...
        for (network in candidates) {
            ...
            if (connection is HttpsURLConnection) {
                if (BuildConfig.OTA_PINNED_CA_ENABLED && shouldUsePinnedCa(url.host)) {
    ...
    private fun shouldUsePinnedCa(host: String?): Boolean {
        val normalized = host.orEmpty().lowercase(Locale.ENGLISH).trim()
        if (normalized.isBlank()) return false
        return normalized == "yasichs.com" || normalized.endsWith(".yasichs.com")
    }
